...
class BoundingBox:
    ...
    @classmethod
    def from_crop_string(cls, crop_str: str) -> Optional['BoundingBox']:
        """Parse CROP(x1,y1,x2,y2) format."""
        match = re.search(r'CROP\((\d+),(\d+),(\d+),(\d+)\)', crop_str)
        if match:
            return cls(
...
def parse_crop_from_output(output: str) -> Optional[BoundingBox]:
    """Extract bounding box from model output, handling extra text."""
    bbox = None
    
    # Look for CROP(x1,y1,x2,y2) pattern
    ...
    if match:
        ...
        # Fallback: look for just numbers that might be coordinates
    ...
    # Validate: reject invalid boxes where x1 >= x2 or y1 >= y2
    if bbox is not None:
...
def evaluate_crop_prediction(
    ...
    rejected_crop: Optional[str] = None
) -> Dict:
    """Evaluate a single crop prediction."""
    # Parse predicted bbox
    pred_bbox = parse_crop_from_output(predicted_output)

    # Parse ground truth bbox
    gt_bbox = BoundingBox.from_crop_string(ground_truth_crop)

    # Parse rejected bbox if available
    rej_bbox = BoundingBox.from_crop_string(rejected_crop) if rejected_crop else None

    result = {
        'prediction': predicted_output.strip(),
        'ground_truth': ground_truth_crop,
        'parsed_success': pred_bbox is not None,
        'exact_match': False,
        'iou': 0.0,
        'better_than_rejected': False,
        'iou_vs_rejected': 0.0
    }

    if pred_bbox and gt_bbox:
        # Check exact match
        result['exact_match'] = str(pred_bbox) == ground_truth_crop

        # Calculate IOU
        result['iou'] = pred_bbox.iou(gt_bbox)

        # Compare to rejected if available
        if rej_bbox:
            iou_rejected = pred_bbox.iou(rej_bbox)
            iou_gt = pred_bbox.iou(gt_bbox)
            result['iou_vs_rejected'] = iou_rejected
            result['better_than_rejected'] = iou_gt > iou_rejected
...
def deduplicate_by_prompt(test_data: List[Dict]) -> List[Dict]:
    """Remove duplicate prompts, keeping the first occurrence of each unique prompt."""
    seen_prompts = set()
    deduplicated = []
...
def test_model(model, tokenizer, test_data: List[Dict], max_samples: Optional[int] = None):
    """Evaluate model on test data with metrics."""
    print("\n" + "=" * 60)
    print("Evaluating VGAP Model Performance")
    print("=" * 60)

    # Deduplicate by prompt to avoid evaluating the same prompt multiple times
    # (since DPO creates 3 preference pairs per original sample)
    original_count = len(test_data)
    test_data = deduplicate_by_prompt(test_data)
    deduplicated_count = len(test_data)

    print(f"Deduplicated prompts: {original_count} → {deduplicated_count} unique prompts")
    ...
    for i, sample in enumerate(test_data):
        ...
        # Evaluate prediction
        eval_result = evaluate_crop_prediction(output, ground_truth, rejected)
        eval_result['sample_id'] = i
        results.append(eval_result)

        if eval_result['parsed_success']:
            successful_parses += 1

    # Compute aggregate metrics
    metrics = compute_crop_metrics(results)

    # Print results
    print(f"\n{'=' * 60}")
    print("EVALUATION RESULTS")
    print(f"{'=' * 60}")
    print(f"{'Samples evaluated':<25} {len(results):>10}")
    print(f"{'Successful parses':<25} {successful_parses/len(results):>9.1%} ({successful_parses}/{len(results)})")
    print(f"{'Exact matches':<25} {metrics['exact_match_rate']:>9.1%}")
    print(f"{'Mean IOU':<25} {metrics['mean_iou']:>10.3f}")
    print(f"{'IOU > 0.5':<25} {metrics['iou_above_05']:>9.1%}")
    print(f"{'IOU > 0.7':<25} {metrics['iou_above_07']:>9.1%}")
    print(f"{'Better than rejected':<25} {metrics['better_than_rejected']:>9.1%}")
    ...
    for i in range(min(5, len(results))):
        ...
        print(f"IOU: {result['iou']:.3f} | Exact: {result['exact_match']} | Better: {result['better_than_rejected']}")

    return results, metrics


def compute_crop_metrics(results: List[Dict]) -> Dict:
    """Compute aggregate metrics from evaluation results."""
    if not results:
        return {}

    # Filter successful parses
    successful = [r for r in results if r['parsed_success']]
    ...
    if not successful:
        return {
            ...
            'iou_above_05': 0.0,
            'iou_above_07': 0.0,
            'better_than_rejected': 0.0,
            'parse_success_rate': 0.0
        }

    return {
        'parse_success_rate': len(successful) / len(results),
        'exact_match_rate': sum(r['exact_match'] for r in successful) / len(successful),
        'mean_iou': sum(r['iou'] for r in successful) / len(successful),
        'iou_above_05': sum(r['iou'] >= 0.5 for r in successful) / len(successful),
        'iou_above_07': sum(r['iou'] >= 0.7 for r in successful) / len(successful),
        'better_than_rejected': sum(1 for r in successful if r['better_than_rejected']) / len(successful)
    }
...
Examples:
  ...
  python train_vgap.py \\
    ...
    parser.add_argument("--eval_samples", type=int, default=50, help="Number of samples to evaluate (default: 50)")
    ...
    # Load raw data for testing/evaluation
    ...
    if args.load_model:
        ...
        results, metrics = test_model(model, tokenizer, raw_data, max_samples=args.eval_samples)
        print("\n✓ Evaluation complete!")
        return
    ...
    if args.test_only:
        ...
        results, metrics = test_model(model, tokenizer, raw_data, max_samples=20)
    else:
        # Train
        trainer = train_dpo(
            model=model,
        ...
        print("\n" + "=" * 60)
        print("Evaluating trained model (loading fresh for clean inference)...")
        print("=" * 60)
        ...
        results, metrics = test_model(trained_model, trained_tokenizer, raw_data, max_samples=20)  # Quick eval after training
