...
async def get_element_data(element, tag_name,viewport_size,seen_elements=[]):
    try:
        ...
        # Get ALL attributes like in VGAP training data (id, class, aria, etc.)
        all_attributes = {}
        # Common attributes from training examples
        attr_names = ['id', 'class', 'aria-label', 'aria-describedby', 'aria-role',
                     'role', 'type', 'name', 'title', 'alt', 'href', 'src', 'value']

        for attr in attr_names:
            value = await element.get_attribute(attr, timeout=0)
            if value:
                all_attributes[attr] = value

        # await aprint("start to get element description",element,tag_name )
        description = await get_element_description(element, real_tag_name, all_attributes.get('role'), all_attributes.get('type'))
        # print(description)
        if not description:
            return None

        # Build tag with ALL attributes (matching VGAP training format)
        attr_parts = []
        for attr, value in all_attributes.items():
            # Format like: id="bookCarTab" class="app-components-..."
            attr_parts.append(f'{attr}="{value}"')

        if attr_parts:
            tag_head += " " + " ".join(attr_parts)
...
async def get_interactive_elements_with_playwright(page,viewport_size):
    ...
    for selector in interactive_elements_selectors:
        ...
        total_initial_count += element_count
        print(f"DEBUG: Found {element_count} '{selector}' elements")

    print(f"DEBUG: Total initial interactive elements: {total_initial_count}")
    ...
    print(f"DEBUG: After first pass filtering: {len(interactive_elements)} elements")
    ...
    # Count all elements in second pass
    total_second_pass = 0
    for selector in interactive_elements_selectors:
        locator = page.locator(selector)
        element_count = await locator.count()
        total_second_pass += element_count
        print(f"DEBUG: Second pass found {element_count} total elements")

    print(f"DEBUG: Total elements in second pass: {total_second_pass}")
    ...
    print(f"DEBUG: Final filtered elements: {len(interactive_elements)}")
