...
def format_elements_vgap(elements, viewport_size):
    """
    Format elements for VGAP cropping model.
    Use simplified format that works with the model.
    """
    formatted_elements = []

    for i, element in enumerate(elements):
        # Get bounding box in pixel coordinates
        box_raw = element.get("box_raw", [0, 0, 0, 0])
        if len(box_raw) == 4:
            x, y, width, height = box_raw
            # Use integers, no decimals
            bbox_str = f"bbox:({int(x)},{int(y)},{int(x+width)},{int(y+height)})"
        else:
            bbox_str = "bbox:(0,0,100,100)"

        # Use simplified HTML format - just tag and basic attributes
        tag = element.get("tag", "div")
        tag_with_role = element.get("tag_with_role", tag)

        # Remove the extra attributes we added, keep it simple like training data
        # Split by spaces and take only the first part (the tag itself)
        simple_tag = tag_with_role.split()[0] if tag_with_role else tag

        # Get description but truncate it significantly like training data
        description = element.get("description", "")
        if len(description) > 50:  # Much shorter than our previous 30 words
            description = description[:50] + "..."

        element_str = f"[{i+1}] <{simple_tag}>{description}</{tag}> {bbox_str}"
