...
from .demo_utils.format_prompt import format_choices, format_elements_vgap, postprocess_action_lmm, postprocess_action_lmm_pixel
...
class SeeActAgent:
    def __init__(self,
                 ...
                 model="gpt-4o",
                 temperature=0.9,
                 use_vgap_cropping=False,
                 vgap_model="ollama/vgap-2k-v1:latest",
                 vgap_max_retries=3
                 ):
            ...
            if config_path is not None:
                ...
                config = {
                    ...
                    "openai": {
                        ...
                        "use_vgap_cropping": use_vgap_cropping,
                        "vgap_model": vgap_model,
                        "vgap_max_retries": vgap_max_retries
                    }
                }
            config.update({
                "browser": {
    ...
    def _query_vgap_model(self, prompt):
        """Query VGAP model for crop coordinates"""
        try:
            vgap_model = self.config['openai']['vgap_model']
            if vgap_model.startswith("ollama/"):
                # Use Ollama API directly
                ollama_model = vgap_model.replace("ollama/", "")
                ...
                from .demo_utils.inference_engine import LiteLLMEngine
                vgap_engine = LiteLLMEngine(model=vgap_model)
                return vgap_engine.generate(prompt=[prompt, "", ""])
        except Exception as e:
            self.logger.error(f"VGAP query failed: {e}")
            return None

    def _extract_crop_coordinates(self, response):
        """Extract CROP coordinates from VGAP response"""
        if not response:
            return None

        # Strategy 1: Direct CROP(x,y,x,y) format (primary - this is what VGAP should complete)
        crop_match = re.search(r'CROP\((\d+(?:\.\d+)?),(\d+(?:\.\d+)?),(\d+(?:\.\d+)?),(\d+(?:\.\d+)?)\)', response)
        if crop_match:
            return tuple(map(float, crop_match.groups()))

        # Strategy 2: Just the coordinates after "CROP("
        # Look for pattern like: CROP(123.45, 67.89, 234.56, 89.01)
        simple_crop = re.search(r'CROP\(\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*\)', response)
        if simple_crop:
            return tuple(map(float, simple_crop.groups()))

        # Strategy 3: bbox(...) format (fallback, allowing spaces)
        bbox_match = re.search(r'bbox[:\(]\(\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)\s*\)', response)
        if bbox_match:
            return tuple(map(float, bbox_match.groups()))

        # Strategy 4: coordinate patterns with floats
        coord_match = re.search(r'x=(\d+(?:\.\d+)?),y=(\d+(?:\.\d+)?),x2=(\d+(?:\.\d+)?),y2=(\d+(?:\.\d+)?)', response)
        if coord_match:
            return tuple(map(float, coord_match.groups()))

        # Strategy 5: Look for any 4 consecutive numbers (including floats)
        numbers = re.findall(r'\b(\d+(?:\.\d+)?)\b', response)
        if len(numbers) >= 4:
            candidates = [float(n) for n in numbers[:8]]
    ...
    def _crop_screenshot(self, screenshot_path, crop_coords, output_path):
        """Crop screenshot to specified coordinates"""
        try:
            with Image.open(screenshot_path) as img:
                x1, y1, x2, y2 = crop_coords
                # Convert floats to ints and ensure coordinates are within image bounds
                ...
                if x2 - x1 < 10 or y2 - y1 < 10:
                    self.logger.warning(f"Crop region too small ({x2-x1}x{y2-y1}), expanding...")
                    center_x, center_y = (x1 + x2) // 2, (y1 + y2) // 2
                    half_width, half_height = max(50, (x2 - x1) // 2), max(50, (y2 - y1) // 2)
...
12. When there are multiple clickable buttons having the same value, choose the one with less obstacles in the screenshot.            
    ...
    async def predict(self):
        ...
        self.logger.info(f"FULL SCREENSHOT: {screenshot_path}")
        try:
        ...
        if self.config['openai'].get('use_vgap_cropping', False):
            self.logger.info("🔍 VGAP CROPPING ENABLED - Analyzing webpage for optimal crop region...")

            # Format elements for VGAP (numbered format with pixel bboxes)
            vgap_elements = format_elements_vgap(elements, self.config['browser']['viewport'])
            vgap_prompt = f"Task: {self.tasks[-1]}\nScreen: {self.config['browser']['viewport']['width']}x{self.config['browser']['viewport']['height']}\nCandidates:\n" + "\n".join(vgap_elements) + "\n\nOutput the optimal crop region:"

            # Log VGAP prompt for debugging
            vgap_log_path = os.path.join(self.main_path, 'vgap_logs', f'vgap_prompt_{self.time_step}.txt')
            os.makedirs(os.path.dirname(vgap_log_path), exist_ok=True)
            with open(vgap_log_path, 'w') as f:
                f.write(vgap_prompt)
            self.logger.info(f"VGAP PROMPT LOGGED: {vgap_log_path}")

            # Try VGAP cropping with retries
            max_retries = self.config['openai'].get('vgap_max_retries', 3)
            crop_coords = None

            for attempt in range(max_retries):
                self.logger.info(f"VGAP ATTEMPT {attempt + 1}/{max_retries}")
                vgap_response = self._query_vgap_model(vgap_prompt)

                if vgap_response:
                    self.logger.info(f"VGAP FULL RAW RESPONSE: {vgap_response}")
                    self.logger.info(f"VGAP RESPONSE: {vgap_response[:200]}...")
                    crop_coords = self._extract_crop_coordinates(vgap_response)
                    if crop_coords:
                        self.logger.info(f"✅ VGAP EXTRACTED COORDS: CROP{crop_coords}")
                        break
                    else:
                        self.logger.info("❌ VGAP failed to extract coordinates")
                else:
                    self.logger.info("❌ VGAP query failed")

            # Apply cropping if we got coordinates
            if crop_coords:
                cropped_path = os.path.join(self.main_path, 'screenshots', f'screen_{self.time_step}_cropped.png')
                if self._crop_screenshot(screenshot_path, crop_coords, cropped_path):
                    final_image_path = cropped_path
                    self.logger.info(f"✅ CROPPED IMAGE SAVED: {cropped_path}")
                    self.logger.info(f"   Crop region: {crop_coords[2]-crop_coords[0]}x{crop_coords[3]-crop_coords[1]} pixels")
                else:
                    self.logger.info("❌ Image cropping failed, using full screenshot")
            else:
                self.logger.info("❌ VGAP cropping failed, using full screenshot")
        else:
            self.logger.info("📷 Using full screenshot (VGAP cropping disabled)")

        terminal_width = 10
        self.logger.info(f"Step - {self.time_step}\n{'-' * terminal_width}\nAction Generation ➡️")
        # for prompt_part in prompt:
        self.logger.info("TASK: " + self.tasks[-1])
        self.logger.info("IMAGE: " + final_image_path)
        self.logger.info("Previous:")
        for action in self.taken_actions:
            self.logger.info(action)

        output0 = self.engine.generate(prompt=prompt, image_path=final_image_path, turn_number=0)
